...
def write_dataframe(df):
    ...
    out_df.to_csv('analytics-data.csv',index=False)

def copy_file_to_webserver():
    """Copy images from model runs to webserver where they can be viewed publically."""
    try:
        os.system('scp -i /etc/ssh/keys/pdaniel/scp_rsa {} skyrocket8.mbari.org:/var/www/html/data/web-analytics/ '.format(os.path.join(BASE_DIR,'analytics-data.csv')))
    except:
